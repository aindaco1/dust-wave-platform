...
NAME = "DustWave virtual display test"
...
class NativeTools:
    def __init__(self, build, window=False, screen_probe=None):
        ...
        self.screen_probe = screen_probe or self._appkit_screens
        if screen_probe is None:
            subprocess.run(["clang++", "-std=c++17", "-fobjc-arc", "-framework", "AppKit",
                            str(source / "ScreenProbe.mm"), "-o", str(self.build / "ScreenProbe")],
                           check=True, timeout=60)

    def _appkit_screens(self):
        return json.loads(subprocess.check_output(
            [str(self.build / "ScreenProbe")], text=True, timeout=10))

    def screens(self):
        return self.screen_probe()
    ...
    def display(self, width, height, scale, origin=None):
        ...
        try:
            ...
            try:
                observed = wait_for(configured, "virtual display geometry and scale")
...
def serial_desktop():
    ...
    with open(pathlib.Path(tempfile.gettempdir()) / "owlswitch-native-display.lock", "w") as lock:
        try:
            ...
            raise RuntimeError("Another native display test is running") from error
        yield
