(function () {
  ...
  var policyScript = document.currentScript || document.querySelector('script[data-dustwave-video-posters="true"]');
  function policy(name, fallback, pattern, maxLength) {
    var value = String(policyScript && policyScript.getAttribute(name) || '').trim();
    return value.length <= maxLength && pattern.test(value) && !/^(?:__proto__|prototype|constructor)$/.test(value)
      ? value : fallback;
  }
  var globalName = policy('data-poster-global', 'DustWaveVideoPosters', /^[A-Za-z_$][A-Za-z0-9_$]*$/, 64);
  var cacheKey = policy('data-poster-cache-key', 'dustwave_first_frame_poster', /^[A-Za-z0-9_.:-]+$/, 128);
  var useDocumentBase = policyScript && policyScript.getAttribute('data-poster-url-base') === 'document-base';
  ...
  function cacheBustedSameOriginUrl(src, index) {
    try {
      var baseUrl = useDocumentBase ? document.baseURI || window.location.href : window.location.href;
      var url = new URL(src, baseUrl);
      var baseOrigin = new URL(baseUrl, window.location.href).origin;
      var pageOrigin = !useDocumentBase || (window.location.origin && window.location.origin !== 'null')
        ? window.location.origin
        : baseOrigin;
      if (url.origin !== pageOrigin) return '';
      url.searchParams.set(cacheKey, String(Date.now()) + '-' + String(index));
      return url.href;
    } catch (_error) {
      return '';
    }
