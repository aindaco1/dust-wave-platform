(function() {
  ...
  var policyScript = document.currentScript || document.querySelector('script[data-dustwave-cart-icon="true"]');

  function boundedPolicyValue(value, fallback, pattern, maximum) {
    var normalized = String(value || '').trim();
    return normalized && normalized.length <= maximum && pattern.test(normalized)
      ? normalized
      : fallback;
  }

  var cacheKey = boundedPolicyValue(
    policyScript?.dataset?.cartCacheKey,
    'dustwave_cart_cache',
    /^[A-Za-z0-9_.:-]+$/,
    128
  );
  var providerGlobal = boundedPolicyValue(
    policyScript?.dataset?.cartProviderGlobal,
    'DustWaveCartProvider',
    /^[A-Za-z_$][A-Za-z0-9_$]*$/,
    64
  );
  var providerReadyEvent = boundedPolicyValue(
    policyScript?.dataset?.cartProviderReadyEvent,
    'dustwavecart.provider.ready',
    /^[A-Za-z0-9_.:-]+$/,
    128
  );
  var cartReadyEvent = boundedPolicyValue(
    policyScript?.dataset?.cartReadyEvent,
    'dustwavecart.ready',
    /^[A-Za-z0-9_.:-]+$/,
    128
  );
  ...
  function maybeBindProvider() {
    var provider = window[providerGlobal];
    if (!provider) return;
  ...
  maybeBindProvider();
  document.addEventListener(providerReadyEvent, maybeBindProvider);
  document.addEventListener(cartReadyEvent, markLoaded);
})();
