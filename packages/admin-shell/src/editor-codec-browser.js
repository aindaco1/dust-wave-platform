(function(scope) {
  "use strict";
  if (scope.DustWaveAdminShellEditorCodec) return;
const TEXT_NODE = 3;
...
const TIMED_TEXT_POLICY = Object.freeze({
  blockMode: false,
  allowHeadings: false,
  allowLinks: false,
  allowLists: false
...
function renderEditorInlineMarkdown(value, policy = RICH_TEXT_POLICY) {
  let html = escapeEditorHtml(String(value || "").replace(/\u0000/g, ""));
  ...
  if (policy.allowLinks) {
    html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match, label, href) => {
      ...
      const safe = (policy.isSafeHref || isSafeEditorHref)(normalizedHref);
      if (!safe) {
        policy.onUnsafeHref?.(normalizedHref);
        return policy.discardUnsafeLinks ? label : match;
      }
      const attributes = policy.externalLinks && /^https?:\/\//i.test(normalizedHref)
        ? ' target="_blank" rel="noopener noreferrer"' : '';
      const link = `<a href="${escapeEditorAttribute(normalizedHref)}"${attributes}>${renderInlineEmphasis(label)}</a>`;
      links.push(link);
      return `\u0000LINK${links.length - 1}\u0000`;
    });
  }
  return renderInlineEmphasis(html).replace(/\u0000LINK(\d+)\u0000/g, (_, index) => links[Number(index)]);
}

function markdownToEditorHtml(value, policy = RICH_TEXT_POLICY) {
...
function nodeToMarkdown(node, policy = RICH_TEXT_POLICY) {
  if (!node) return "";
...
function editorElementToMarkdown(control, policy = RICH_TEXT_POLICY) {
  const blocks = [];
  ...
  Object.defineProperty(scope, "DustWaveAdminShellEditorCodec", { value: Object.freeze({ RICH_TEXT_POLICY, TIMED_TEXT_POLICY, escapeEditorHtml, escapeEditorAttribute, isSafeEditorHref, renderEditorInlineMarkdown, markdownToEditorHtml, normalizePastedPlainText, sanitizeClipboardHtml, sanitizeClipboardPayload, nodeToMarkdown, editorElementToMarkdown }), configurable: false });
})(globalThis);
