...
public enum LocalSpeechOfflinePolicy {
...
public actor ParakeetTranscriber {
    ...
    public func transcribe(
    ...
    ) async throws -> ParakeetTranscriptResult {
        ...
        if let progress, audioDuration > 15 {
            let stream = await manager.transcriptionProgressStream
            progressTask = Task {
                do {
                    for try await value in stream {
                        await progress(min(1, max(0, value)))
                    }
                } catch {
                    // The transcription call below owns and reports the underlying failure.
                }
            }
        } else {
            progressTask = nil
